...
type TMergeWithReturn<O extends (T extends TObjectJSONSchema ? TObjectJSONSchema : Partial<T>), T extends TJSONSchema = TJSONSchema> = O extends TObjectJSONSchema<infer U> ? T extends TObjectJSONSchema<infer V> ? TObjectJSONSchema<U & V> & Omit<T, 'required' | 'properties'> : TObjectJSONSchema<U> & Omit<T, 'required' | 'properties'> : T & O;
